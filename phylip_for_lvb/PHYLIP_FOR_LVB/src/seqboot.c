...
extern FILE *infile, *outfile, *catfile, *weightfile; 
extern long spp;
extern naym *nayme;
extern boolean dotdiff;
extern boolean interleaved, printdata, weights;
extern steptr weight;
...
Char infilename[100], outfilename[100], catfilename[100], outcatfilename[100],
   weightfilename[100], outweightfilename[100];
long sites, loci, maxalleles, groups, newsites, newersites,
             newgroups, newergroups, nenzymes, reps, ws, blocksize, categs;
boolean permute, jackknife, weights, categories, factors, enzymes, all,
	justwts, progress;
datatype data;
...
void getoptions()
{
  ...
  for (;;) {
    ...
    if (ch == 'Y')
	   break;
    if (strchr("ABCDEFJRWI1.20",ch) != NULL){
      switch (ch) {

...
/* Version 3.6.  Copyright (c) 1986-2000 by the University of Washington.
  ...
  Daniel Fineman, and Akiko Fuseki.
...
char showparms()
{
  ...
  switch (plotter) {
    ...
    case other:
      ...
      break;
    default:	/*case xpreview	not handl t handled	*/
      break;
  }
  ...
  if  (style == circular) {
    for (i = 0; i < nextnode; i++) {
      if (nodep[i]->tip) {
        ...
        if (cos(angle) * cos(nodep[i]->oldtheta) +
            sin(angle) * sin(nodep[i]->oldtheta) > 0.0) {
          if (sin(angle) > 0.0)
...
void plottree(node *p, node *q)
{
  ...
  if (p != root) {
    ...
    case curvogram:
      plot(penup, x1, y1) ;
      curvespline(x1,y1,x2,y2,grows == vertical,20);
      break;
  ...
  for (i = 0; i < (nextnode); i++) {
    if (nodep[i]->tip) {
      ...
      if (style == circular) {
        ...
        left = !right;
        if (right) {
          dx = labelheight * expand * cos(labangle);
          dy = labelheight * expand * sin(labangle);
          dx += labelheight * expand * 0.5 * (firstlet[i]
                                               * cos(labangle + 9*pi/8.0));
          dy += labelheight * expand * 0.5 * (firstlet[i]
                                               * sin(labangle + 9*pi/8.0));
        }
        if (left) {
          dx = - labelheight * expand * cos(labangle);
          dy = labelheight * expand * sin(labangle);
        ...
        dx = labelheight * expand * -0.70710 * cos(labangle + pi / 4.0);
        dy = labelheight * expand * (1.0 - 0.70710 * sin(labangle + pi / 4.0));
      }
      if (style == circular) 
...
void setup_environment(Char *argv[], boolean *canbeplotted)
{
  ...
  treeread (intree, &root, treenode, &goteof, &firsttree,
            nodep, INTREE, "DrawGram", &nextnode, &haslengths,

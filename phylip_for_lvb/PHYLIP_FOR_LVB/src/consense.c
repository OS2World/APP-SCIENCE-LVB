...
extern FILE *outfile, *intree, *intree2;
extern long spp;
...
extern FILE *outtree;
extern Char outfilename[100], intreename[100], intree2name[100], outtreename[100];
extern node *root;
...
extern boolean trout, firsttree, noroot, outgropt, didreroot, prntsets,
               progress, treeprint, goteof, ibmpc, ansi, strict, mr, mre, ml;
...
void getoptions()
{
  ...
  do {
    ...
    printf(" C   Consensus type (strict, MR, MRe, Ml):");
    if (strict)
      printf("  strict\n");
    else if (mr)
...
{  
  ...
  ntrees = 0.0;
  maxgrp = 32767;   /* initial size of set has table */
  lasti  = -1;
  ...
  /* Read the tree file and put together grouping, order, and
     timesseen */
  read_groups (&pattern_array, timesseen_changes, &trees_in,
                 intree, prntsets, INTREE, "Consense");
  ...
  if (trout) {
    treeout(root);
    if (progress)
      printf("Consensus tree written to output tree file\n\n");
  }
  if (progress)

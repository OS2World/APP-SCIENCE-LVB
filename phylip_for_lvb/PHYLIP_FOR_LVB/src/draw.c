...
static char *figfonts[] = {"Times-Roman","Times-Italic","Times-Bold","Times-BoldItalic",
...
extern boolean       dotmatrix,empty,preview,previewing,pictbold,pictitalic,
                     pictshadow,pictoutline,ansi,ibmpc;
...
void   plotrparms(long);
void   getplotter(void);
void   makeprevwindow(void);
void   changepen(pentype);
...
void initplotter(long ntips, char *fontname)
{
 ...
 switch (plotter) {
  ...
  case bmp:
    ...
    full_pic     = (byte *) calloc (((padded_width *2) * (int) ysize), 1) ;
    break ;
  ...
  case mac:
    ...
    if (pictint == 0)
         pictint=1;
    PenSize((int)pictint/2,(int)pictint/2);
#endif
    break;
...
void plotrparms(long ntips)
{
  ...
  case bmp:            /* since it's resolution dependent, make 1x1 pixels  */
    ...
    strpwide = (long)(xsize * xunitspercm) + 1;
    break;
...
void makeprevwindow()
{ /* open the preview window on the desktop */
#ifdef X
    ...
    case xpreview:
      ...
      if (!mainwin) {
        if ((display=XOpenDisplay(getenv("DISPLAY"))) == NULL) {
          fprintf(stderr,"%s:  Can't open X Windows display.\n", progname);
          fprintf(stderr, "You requested display: %s\n", getenv("DISPLAY"));
          fprintf(stderr, "Make sure you are in X Windows and that the\n");
          fprintf(stderr, "DISPLAY environment variable is set correctly.\n");
          fprintf(stderr, "If do not have X Windows on your current\n");
          fprintf(stderr, "screen, either run from one that does, or change\n");
          fprintf(stderr, "the V (preview) option in the program menu.\n");
          exxit(-1);
        }
        fontrsc = XGetDefault(display,progname,"font");
        if (fontrsc)
          strcpy(fontname,fontrsc);
        if ((fontst = XLoadQueryFont(display,FONT)) == NULL) {
        ...
        height -= 200;
        width -= 200;
        if (((double)width)/((double)height) > xsize/ysize)
          width = (xsize/ysize) * height;
        else
          height = (ysize/xsize) * width;
        mainwin=XCreateSimpleWindow(display,DefaultRootWindow(display),x,y,
                  width,height,4,BlackPixel(display,DefaultScreen(display)),
                                    WhitePixel(display,DefaultScreen(display)));
        if (!mainwin) {
          fprintf(stderr,"Can't create window, for some reason\n");
          exxit(-1);
        }
        ...
        XStoreName(display,mainwin,progname);
        XSetIconName(display,mainwin,progname);
...
#endif
      break;
    default:	/* most cases not handled		*/
      break;
   }
} /* makeprevwindow */
...
for (;;){
  ...
  if (inmetrics){
    scanned=sscanf(line,"%*s %s %*s %*s %s",word1,word2);
    charnum = atoi(word1);
    charlen = atoi(word2);
    nmetrics--;
    if (nmetrics == 0)
			...
			long numlines, node *root)
{
  ...
  if (dotmatrix){
	...
	/* large stripe, do in DEFAULT_STRIPE_HEIGHT (20)-line     */
        for (i=0;i<strpdeep;++i){
	  swap(&stripe[i%DEFAULT_STRIPE_HEIGHT],
	       &stripe[i]);
...
#define min4(a,b,c,d) (min(min(a,b),min(c,d)))
  ...
  /* This just changed to make -Wall happy, */
  if ((strcmp(fontname,"Hershey") == 0)||
 ...
 /* print native font.  idraw, PS, pict, and fig. */
   ...
   TextFace((int)((pictbold ? 1: 0) | (pictitalic ? 2 : 0)|
      ...
      MoveTo((int)floor((double)x + 0.5),
              winheight - (long)floor((double)y + 0.5)); 
      putstring(pstring);
    TextFont(macfontid("courier"));
    TextSize(10);
    TextFace(0);
...
/* fn--fontname| xo,yo--x and y offsets */
{
  ...
  if (previewer != winpreview && previewer != mac) {
...
long allocstripe(striptype stripe, long x, long y)
{
  long i;
  for (i=0 ; i<=y; ++i){
    stripe[i] = (MALLOCRETURN *)calloc((x+1)*sizeof(Char),1);
    if (!stripe[i])
      break;
  }
  return i-1; 

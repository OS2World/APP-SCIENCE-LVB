...
Char infilename[100], outfilename[100], intreename[100];
...
void regressions()
{
  ...
  for (i = 0; i < chars; i++) {
    for (j = 0; j < chars; j++)
      fprintf(outfile, "%10.4f", sumprod[i][j]);
    putc('\n', outfile);
  }
  ...
  for (i = 0; i < chars; i++) {
    for (j = 0; j < chars; j++)
      fprintf(outfile, "%10.4f", sumprod[i][j] / sumprod[i][i]);
    putc('\n', outfile);
  }
  fprintf(outfile, "\nCorrelations\n");
  fprintf(outfile, "------------\n\n");
...
/* print out ML covariances and regressions in the error-covariance case */
   ...
   if (!novara) {
     ...
     for (i = 0; i < chars; i++) {
       for (j = 0; j < chars; j++)
         fprintf(outfile, "%10.4f", vara[i][j] / vara[i][i]);
       putc('\n', outfile);
     }
     fprintf(outfile, "\n");
   ...
   for (i = 0; i < chars; i++) {
     for (j = 0; j < chars; j++)
       fprintf(outfile, "%10.4f", vare[i][j] / vare[i][i]);
     putc('\n', outfile);
   }
   fprintf(outfile, "\n");
   fprintf(outfile, "\nVarE Correlations\n");
...
/* EM iteration of error and phylogenetic covariances */
   ...
   do {
     ...
     if (its % 100 == 0)
      printf("Iteration no. %ld:  lnL = %f, Norm = %f\n", its, logL, relnorm);
     its++;
   } while (relnorm > 0.00001);
} /* emiterate */
...
void maketree()
{
  ...
  alloctree(&curtree.nodep, nonodes, true);
  setuptree(&curtree, nonodes, true);
  which = 1;
  while (which <= numtrees) {
    ...
    treeread (intree, &curtree.start, curtree.nodep, &goteof, &first,
            curtree.nodep, intreename, "Contrast", &nextnode, &haslengths,
            &grbg, initcontrastnode);
    q = curtree.start;
    r = curtree.start;

...
void doinit()
{
  ...
  inputnumbers(&spp, &loci, &nonodes2, 2);
  getoptions();
  if (printdata)
...
void getalleles()
{
  ...
  if (contchars ) {
    ...
    for (i = 1; i <= loci; i++) {
      ...
      if (alleles[i - 1] <= 0) {
        printf("BAD NUMBER OF ALLELES: %4ld AT LOCUS %3ld\n", alleles[i - 1], i);
        exxit(-1);
      }
  ...
  for (i = 0; i < spp; i++) {
	...
	for (k = 1; k <= n; k++) {
	  ...
	  if (!contchars && x[i][m - 1] < 0.0) {
	    printf("\nLOCUS%3ld IN SPECIES%3ld: AN ALLELE", j, i + 1);
	...
	if (all && fabs(sum - 1.0) > epsilon2) {
	  printf("\nLOCUS%3ld IN SPECIES%3ld: FREQUENCIES DO NOT ADD UP TO 1\n",
                   j, i + 1);
          exxit(-1);
          }
	if (!all && !contchars) {
	  ...
	  if (x[i][m - 1] < 0.0) {
		...
		x[i][l] /= sqrt(sum);
	      ...
	      printf("\n LOCUS %3ld IN SPECIES% 3ld: ", j, i + 1);
...
void rearrange(node *p)
{
  ...
  if (!p->tip && !p->back->tip) {
    ...
    addtraverse(r, q->next->back, global && nextsp == spp);
    addtraverse(r, q->next->next->back, global && nextsp == spp);
...
void maketree()
{
  ...
  if (usertree) {
    ...
    if (numtrees > 1 && loci > 1 ) {
      ...
      for (i = 1; i <= num; i++) {
        fprintf(outfile, "%3ld%12.5f", i, l0gl[i - 1]);
        if (maxwhich == i)
    ...
    if (jumb == 1) {
      setuptree(&curtree, nonodes2, usertree);
      setuptree(&priortree, nonodes2, 0);
      setuptree(&bestree, nonodes2, 0);
      if (njumble > 1) 
	setuptree(&bestree2, nonodes2, 0);
    }
    for (i = 1; i <= spp; i++)
      enterorder[i - 1] = i;
    if (jumble)

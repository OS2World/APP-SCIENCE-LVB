...
/* Version 3.6.  Copyright (c) 1986-2000 by the University of Washington and
...
extern FILE *intree;
extern boolean ansi, ibmpc;
extern void plotrparms() ;
extern void getplotter() ;
extern void getpreview() ;
extern void plot() ;
extern void changepen() ;
extern void plottext() ;
extern void loadfont() ;
extern long allocstripe() ;
extern boolean plotpreview() ;
extern void initplotter() ;
extern void drawit() ;
extern void finishplotter() ;
extern void clearit() ;
extern boolean plot_without_preview();
...
/* never called
void ntrav2(q, p, p0, sum)
node* q, p, p0;
double sum;
{
  node *qq;

* added for testing - danieyek 990125 *
printf("In ntrav2()!\n");
  * ?? here compute pressure of this node on other one ?? *
  * accumulate pressures where? available: q, p, p0, sum *
  * also possible angles of motion, maxleft, maxright *
  qq = q->next;
  do {
    ntrav2(qq->back,p,p0,sum);
    qq = qq->next;
  }
  while (((q == root) && (qq != q->next)) || ((q != root) && (qq != q)));
* added for testing - danieyek 990125 *
printf("Leaving ntrav1()!\n");
} * ntrav2 */


/*void ntrav1(p, p0, sum)
node* p, p0;
double* sum;
{
  * added ";" - danieyek 990118 *
  node *pp;

* added for testing - danieyek 990125 *
* printf("In ntrav1()!\n"); *
* changed "->" to "." - danieyek 990118 *
  ntrav2(p0.back, p0, sum);
  pp = p->next;
  do {
    ntrav1(pp->back, p0, sum);
    pp = pp->next;
  }
  while (((p == root) && (pp != p->next)) || ((p != root) && (pp != p)));
* added for testing - danieyek 990125 *
...
/* added - danieyek 990226 */
/* Calculating angles at the pivot point */
/* Abandoned; see branchLRHelper() helper function */
void branchLeftRightAngles(node *pPivotSubNode, double *pBrLeftAngle, 
		double *pBrRightAngle)
{
}
...
{
  long i;
  double compr, dx = 0, dy = 0, labangle;
  boolean left, right;
  node *lp;
    ...
    if (nodep[i]->tip) {
      ...
      if (right) {
        if (labeldirec == middle) {
          ...
          dy = -0.3;
          if ((0.5*textlength[i]+0.5)*fabs(sin(nodep[i]->oldtheta)) < 1) {
            dx += (0.5 * textlength[i] + 0.5) * labelheight * expand;
            dy += (0.5*textlength[i]+0.5)*sin(nodep[i]->oldtheta)
                    /cos(nodep[i]->oldtheta) * labelheight * expand;
          }
          else {
            if (sin(nodep[i]->oldtheta) > 0.0) {
              dx += cos(nodep[i]->oldtheta)/sin(nodep[i]->oldtheta)
                     * labelheight * expand;
          ...
          dx += labelheight * expand * 0.5 * (-firstlet[i] * cos(labangle)
                                              + cos(labangle - pi/2.0));
          dy += labelheight * expand * 0.5 * (-firstlet[i] * sin(labangle)
                                              + sin(labangle - pi/2.0));
        }
      }
      if (left) {
  ...
  while (!canbeplotted) {
    ...
    if (preview)
      ...
      canbeplotted=plot_without_preview(fontname,&xoffset,&yoffset,
					&scale,spp,root);
     if ((previewer == winpreview) && (winaction == quitnow))
      canbeplotted = true;
 }
} /* user_loop */


void setup_environment(argc, argv)
int argc;
Char *argv[];
{
  ...
  treeread (intree, &root, treenode, &goteof, &firsttree,
            nodep, INTREE, "DrawTree", &nextnode, &haslengths,

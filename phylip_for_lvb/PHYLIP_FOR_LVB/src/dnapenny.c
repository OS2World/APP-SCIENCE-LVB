...
extern long spp, nonodes, endsite, outgrno, nextree;
...
node *root, *p;
long *zeros;
long chars, howmany, howoften, col, datasets, ith;
...
void supplement(node *r)
{
  ...
  long i, j, k, sum=0, sumall=0, sumadded=0;
...
{  /* Penny's branch-and-bound method for DNA sequences */
  ...
  for (ith = 1; ith <= datasets; ith++) {
    ...
    maketree();
    free(threshwt);
    freenumsteps(nonodes, treenode);
    freebase(nonodes, treenode);
  }
  FClose(infile);
  FClose(outfile);
  FClose(outtree);

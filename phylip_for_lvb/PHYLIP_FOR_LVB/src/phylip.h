...
#if !LVB || !defined(VERSION)	/* lvb.h has its own VERSION macro - DB */
#define VERSION "3.6"
#endif	/* !LVB */
...
#if !LVB	/* for LVB, want to avoid system-dependent code - DB */
...
#if !LVB && defined(__CMS_OPEN) /* LVB tests not planned on CMS - DB */
#define CMS
#define EBCDIC true
...
#else
#define EBCDIC false
#if LVB	/* use LVB 1.0-compatible matrix file name */
#ifdef MATFNAM
#define INFILE MATFNAM
#else
#error LVB matrix file name not defined
#endif	/* MATFNAM */
#else
...
#if LVB	/* for LVB, assume ANSI C - DB */
...
/* includes: */
#if !LVB && defined(UNIX)	/* LVB assumes the ANSI header - DB */
#include<strings.h>
#else
#include<string.h>
#endif	/* !LVB && defined(UNIX) */

#include <math.h>
#include <ctype.h>

#if !LVB && defined(MAC)	/* want avoid system-dep. code - DB */
/* debug beerli */
#ifdef DRAW
#include "interface.h"
#else
...
/* K&R says that there should be a plus in front of the number, but no
   machine I've seen actually uses one; we'll include it just in
   case. */
...
#define maxuser         100  /* maximum number of user-defined trees    */
...
int    eoln(FILE *);
int    filexists(char *);
void   get_command_name (char *, char *);
void   getstryng(char *);
void   openfile(FILE **,char *,char *,char *,char *,char *);
void   cleerhome();
...
void   readoptions(long *, Char *);
void   matchoptions(Char *, Char *);
void   inputweights(long, steptr, boolean *);
void   inputweights2(long, long, long *, steptr, boolean *, char *);
void   printweights(FILE *, long, long, steptr, Char *);

void   inputcategs(long, long, steptr, long, char *);
void   printcategs(FILE *, long, steptr, Char *);
void   inputfactors(long, Char *, boolean *);
void   printfactors(FILE *, long, Char *, Char *);
void   headings(long, char *, char *);
void   initname(long);
...
void   malloc_pheno(node *, long, long, long);
void   malloc_ppheno(node *, long, long, long);
...
void   treeread (FILE *, node **, pointarray, boolean *, boolean *,
		pointarray, char *, char *, long *, boolean *, node **, initptr);
...
void   treeread2 (FILE *, node **, pointarray, boolean, double *,
		boolean *, char *, char *, boolean *, long *);
void   free_x (phenotype, long);
void   free_x_of_ring (node *, long);
void   exxit (int);
/* function prototypes */

...
extern long spp, endsite, nonodes;
...
boolean freqsfrom, jukes, kimura, logdet, gama, lower, f84, weights, progress,
               ctgry, mulsets, justwts, firstset;
node **nodep;
double xi, xv, ttratio, ttratio0, freqa, freqc, freqg, freqt, freqr,
      freqy, freqar, freqcy, freqgr, freqty, cvi, sumrates, fracchange;
extern steptr category;
steptr oldweight;
...
void getoptions()
{
  ...
  for (;;) {
    ...
    if (strchr("CFGWLDTMI012",ch) != NULL){
     ...
     case 'C':
       if (logdet)
         printf(" WARNING: CANNOT SET CATEGORIES FOR LogDet DISTANCE\n");
       else {
         ctgry = !ctgry;
         if (ctgry) {
           initcatn(&categs);
     ...
     case 'T':
       if (jukes) {
         ...
         printf(" TRANSVERSION RATIO FOR JUKES-CANTOR DISTANCE\n");
       } else {
         ttr = !ttr;
         if (ttr)
          initratio(&ttratio);

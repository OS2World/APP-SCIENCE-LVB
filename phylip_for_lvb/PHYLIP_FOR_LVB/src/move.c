...
void getoptions()
{
  ...
  do {
    ...
    printf("  W                 Width of terminal screen?");
    printf("%4ld\n", screenwidth);
	...
	ch = ' ';
      ...
      gotopt = (strchr("OTXPAU0WL",ch) != NULL) ? true : false;
      if (gotopt) {
	switch (ch) {
...
void inputoptions()
{
  ...
  avar = false;
  mix = false;
  readoptions(&extranum, "AFMW");
  for (i = 0; i < (chars); i++)
    weight[i] = 1;
  for (i = 1; i <= extranum; i++) {
    matchoptions(&ch, "AFMW");
    if (ch == 'A') {
      avar = true;
      if (!ancvar) {
...
void buildtree()
{
  ...
  case use:
    ...
    treeread(intree, &root, treenode, &goteof, &firsttree,
                nodep, intreename, "Move", &nextnode, &haslengths,

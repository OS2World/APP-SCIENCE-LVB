...
void   dolmove_drawline(long, long);
...
extern FILE *infile, *intree, *outtree;
extern long spp, chars, nonodes, words, bits;
extern naym *nayme;

Char infilename[100],intreename[100],outtreename[100];
node *root;
...
void getoptions()
{
  ...
  do {
    ...
    printf("  W                Width of terminal screen?%4ld\n",screenwidth);
    do {
      printf(
	"\n\nAre these settings correct? (type Y or the letter for one to change)\n");
      ...
      gotopt = (strchr("TPULA0W",ch) != NULL) ? true : false;
      if (gotopt) {
	switch (ch) {
...
void dolmove_drawline(long i, long lastline)
{
...
void buildtree()
{
  ...
  case use:
    ...
    treeread(intree, &root, treenode, &goteof, &firsttree,
                nodep, intreename, "DolMove", &nextnode, &haslengths,

...
extern long spp, endsite, outgrno, which;
...
long nonodes2, sites, enzymes, weightsum, sitelength, datasets, 
        ith, njumble, jumb=0;
long inseed;
boolean  global, jumble, lngths, weights, trout, trunc8, usertree,
...
void doinit()
{
  ...
  alloctree(&curtree.nodep, nonodes2, 0);
  allocrest();
...
void getinput()
{
  ...
  allocx2(nonodes2, endsite+1, sitelength, curtree.nodep, 0);
  if (!usertree || reconsider) {
...
void rearrange(node *p, node *pp)
{
  ...
  if (!p->tip && !p->back->tip) {
    ...
    if (smoothit)
      ...
      qwhere = q;
    addtraverse(r, p->next->back, global && nextsp == spp);
    addtraverse(r, p->next->next->back, global && nextsp == spp);
...
void inittravtree(node *p)
{
  ...
  if ((!lngths) || (p->v == 0)) { 
    ...
    p->v = 0.75 * (1 - exp(-1.333333 * p->v));
    p->back->v = p->v;
  }
  branchtrans(p->branchnum, p->v);
  p->initialized = false;
  ...
  if (usertree) {
    ...
    while (which <= numtrees) {
      treeread2 (intree, &curtree.start, curtree.nodep,
        lngths, &trweight, &goteof, intreename, "Fitch",
        &haslengths, &spp);
      treevaluate();
      if (reconsider) {
    ...
    standev2(numtrees, weightsum, maxwhich, 1, endsite, maxlogl,

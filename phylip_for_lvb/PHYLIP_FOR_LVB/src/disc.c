...
void standev(long numtrees, long which, long minwhich, double minsteps,
			double *nsteps, double **fsteps)
{ /* compute and write standard deviation of user-defined trees */
  /* used in dollop & mix */
  long i, num, sumw;
  double sum, sum2, sd, TEMP;
  ...
  for (which = 1; which <= num; which++) {
    ...
    if (which == minwhich)
      ...
      for (i = 0; i < (chars); i++) {
        if (weight[i] > 0) {
          sumw += weight[i];
          sum += fsteps[which - 1][i] - fsteps[minwhich - 1][i];

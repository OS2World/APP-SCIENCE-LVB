...
void   old_add_at(node *, node *, node *);
...
void old_add_at(node *below, node *newtip, node *newfork)
{
  /* inserts the nodes newfork and its left descendant, newtip,
    to the tree.  below becomes newfork's right descendant */
  node *leftdesc, *rtdesc;
  ...
  do {
    ...
    if (!done) {
     ...
     j++;
      ...
      add_at(nodep[j - 1], newtip, newfork);
    }
  } while (!done);
  for (i = spp + 1; i < (spp * 2); i++) {
    nodep[i - 1] = nodep[i];
    nodep[i - 1]->index = i;
    nodep[i - 1]->next->index = i;
    nodep[i - 1]->next->next->index = i;
  }
  nonodes = spp * 2 - 1;
}  /* yourtree */
...
treeconstruct.  Memory leak if user reads multiple trees. */
  ...
  case use:
    ...
    treeread(intree, &root, dummy_treenode, &goteof, &firsttree,
             nodep, INTREE, "Retree", &nextnode, &haslengths,
             &grbg, initretreenode);
    nonodes = nextnode;
    treesets[othertree].nodep = treetwo;
    break;
...
void roottreeout(node *p, boolean *userwantsrooted)
{
  /* write out file with representation of final tree */

...
extern FILE *infile, *outfile, *intree, *outtree, *weightfile;
extern long spp, nonodes, endsite, outgrno, nextree, which;
extern boolean ibmpc, ansi, interleaved, printdata, outgropt,
  treeprint, dotdiff;
...
#if LVB || !defined(OLDC)	/* ensure prototypes for LVB - DB */
/* function prototypes */
...
void   grandrearr(boolean);

void   maketree(void);
void  freerest(void);
/* function prototypes */
#endif	/* LVB || !defined(OLDC) */
...
#if LVB
long maxtrees = 100;	/* nontentative def. to positive non-0 value */
#endif	/* LVB */
...
node *grbg;
#if LVB
#define PROGNAMSIZE 50
Char progname[PROGNAMSIZE];
#else
Char progname[20];
#endif	/* LVB */
...
void getoptions()
{
  ...
  for (;;) {
    ...
    if (ch == 'Y')
      break;
    if (strchr("WSVJOTNUMI12345.60",ch) != NULL){
      switch (ch) {
...
void tryadd(node *p, node *item, node *nufork)
{
  ...
  if (!p->tip) {
    ...
    if (p->back) {
      ...
      sumnsteps2(tempsum, temp, p->back, 0, bound, threshwt);
    } else {
      multisumnsteps(temp, tempadd, 0, bound, threshwt);
      tempsum->sumsteps = temp->sumsteps;
    }
    if (tempsum->sumsteps <= -bestyet) {
      if (p->back)
        sumnsteps2(tempsum, temp, p->back, bound+1, endsite, threshwt);
      else {
        multisumnsteps(temp, temp1, bound+1, endsite, threshwt);
        tempsum->sumsteps = temp->sumsteps;
      }
    }
  ...
  if (p == root)
    sumnsteps2(temp, item, p, 0, bound, threshwt);
  else {
    sumnsteps(temp1, item, p, 0, bound);
    sumnsteps2(temp, temp1, p->back, 0, bound, threshwt);
  }
  if (temp->sumsteps <= -bestyet) {
    if (p == root)
      sumnsteps2(temp, item, p, bound+1, endsite, threshwt);
    else {
      sumnsteps(temp1, item, p, bound+1, endsite);
      sumnsteps2(temp, temp1, p->back, bound+1, endsite, threshwt);
    }
  }
...
void grandrearr(boolean glob)
{
  /* calls either global rearrangement or local rearrangement on best trees */
  long treei, j, nextnode;
  boolean done;
  node *dummy;

  done = false;
  do {
    treei = findunrearranged(bestrees, nextree, glob);
    if (treei < 0)
      done = true;
    else {
      if (glob)
...
void maketree()
{
  ...
  if (!usertree) {
    ...
    while (!done && torearrange(bestrees, nextree)) {
      grandrearr(true);
      grandrearr(false);
      done = rearrfirst;
    }
    ...
    while (which <= numtrees) {
      ...
      treeread(intree, &root, treenode, &goteof, &firsttree,
                 nodep, intreename, "DnaPars", &nextnode, &haslengths,
    ...
    if (numtrees > 1 && chars > 1 )
      standev(chars, numtrees, minwhich, minsteps, nsteps, fsteps);
    for (j = 1; j <= maxuser; j++)
      free(fsteps[j - 1]);
    free(fsteps);
  }
  if (jumb == njumble) {
    if (progress) {
      ...
      if (trout) {
        ...
        if (numtrees > 1)
          printf("s");
        printf(" also written onto tree file\n\n");
      }
    }
  }
}  /* maketree */
...
{  /* DNA parsimony by uphill search */
  ...
  /* reads in spp, chars, and the data. Then calls maketree to
   ...
   argv[0] = "Dnapars";
#endif	/* !LVB && defined(MAC) */
#if !LVB && defined(WIN32)
  phySetConsoleAttributes();
  phyClearScreen();
#endif	/* !LVB && defined(WIN32) */
#if LVB
  strncpy(progname, PROGNAM, PROGNAMSIZE - 1);	/* PROGNAM from lvb.h - DB */
#else
  strcpy(progname,argv[0]);
#endif	/* LVB */
#if LVB
  infile = clnopen(INFILE, "r");
#else
  ...
  doinit();
#if LVB
/* Full-scale operation of dnapars not desired. We get the number of
 * species and sites, the species names and the sequences, then return
 * to LVB. */
  inputdata(chars);
  clnclose(infile, INFILE);
  return;
  /* end of dnapars_wrapper() */
#else
  if (weights || justwts)

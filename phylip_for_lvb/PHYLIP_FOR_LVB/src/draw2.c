...
extern double  xnow, ynow, ysize, xsize, yunitspercm;
...
/* Added by Dan F. just to make -Wall happy, */
extern void pout(), plotrparms(), initplotter(), plottree(), plotlabels(), finishplotter() ;
...
extern boolean       dotmatrix,empty,preview,pictbold,pictitalic,
  pictshadow,pictoutline,ansi,ibmpc;
extern double        expand,xcorner,xnow,xsize,xscale,xunitspercm,yunitspercm,
...
/* remember to respect & translate for clipping region, clip{x,y}{0,1} */
  ...
  if (!dotmatrix || previewing) {
	...
	LineTo((int)floor((double)xabs + 0.5),
	       winheight - (long)floor((double)yabs + 0.5));}
      else{
    ...
    case vrml:
      ...
      /* if this is the root node, use the coordinates to define the view point */
      if (firstNodeP-- == 1)
      {
        fprintf(plotfile, "    Viewpoint\n");
        fprintf(plotfile, "    {\n");
        fprintf(plotfile, "      position %f %f 5\n", newx, newy);
      ...
      {/* pen up = continue, line may not end yet. */
        ...
        if (distance >= episilon)
        {
          ...
          fprintf(plotfile, "          {\n");
          fprintf(plotfile, "            material Material { diffuseColor %i %i %i}\n",
		  (int)(255 * colors[vrmlplotcolor-1].red), 
		  (int)(255 * colors[vrmlplotcolor-1].green), 
		  (int)(255 * colors[vrmlplotcolor-1].blue) );
          fprintf(plotfile, "          }\n");
          ...
          fprintf(plotfile, "              appearance Appearance\n");
          fprintf(plotfile, "              {\n");
          fprintf(plotfile, "                material Material { diffuseColor 0 255 255 }\\
n");
          fprintf(plotfile, "              }\n");
          fprintf(plotfile, "              geometry Sphere\n");
          fprintf(plotfile, "              {\n");
...
/* sense is true if line departing from x1,y1 is tangential to x,
  ...
  case pict:
    ...
    if (x2 > x1 && y2 < y1){ swap(x2,x1); swap(y2,y1); sense = !sense; } 
	...
	arcangle = 90;
	 if (dbottom < dtop) {swap(dbottom,dtop);}
    if (dleft> dright) {swap(dleft,dright);}
			...
			double y3, boolean sense, long segs)
{
  ...
  splyne(x2,y2,x3,y3,!sense,segs/4,false,true); 
}  /* swoopspline */
...
void turn_rows (byte *full_pic, int padded_width, int height)
{
  ...
  for (j = 0 ; j < height ; j++) {
    for (i = 0 ; i < midpoint ; i++) {
      ...
      swap (full_pic[(j * padded_width) + i],
...
boolean plot_without_preview(char *fn, double *xo, double *yo,
			double *scale, long nt, node *root)
{

  previewing = false;
  makebox_no_interaction(fn,xo,yo,scale,nt);
  plottree(root, root);
  plotlabels(fn);
  finishplotter();

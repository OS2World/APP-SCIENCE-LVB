...
void freenumsteps(long nonodes, pointarray treenode)
{
...
void freebase(long nonodes, pointarray treenode)
{
  /* used in dnacomp, dnapars, & dnapenny */
  long i, j;
  node *p;
  ...
  while (!(allread)) {
    ...
    while (i <= spp) {
        ...
        while (j < chars && !(eoln(infile) || eoff(infile))) {
          ...
          if ((strchr("ABCDGHKMNRSTUVWXY?O-",charstate)) == NULL){
            printf("ERROR: BAD BASE: %c AT POSITION %5ld OF SPECIES %3ld\n",
                   charstate, j+1, i);
            if (charstate == '.') {
              printf("Note: periods (.) may not be used as gap characters.\n");
              printf("The correct gap character is (-).\n");
	      printf("Also, periods (.) may not be used to represent 'same\n");
	      printf("base as first sequence'. Give the single-letter code\n");
	      printf("for each base of each sequence.\n");
            }
            exxit(-1);
          }
          j++;
      ...
      /* Then, get the first char. after the return.  If it's a space,
         continue getting spaces 'til you run into a non-space
         character.
         */
...
#if LVB
      /* Push the non-blank character back */
#else
...
void findoutgroup(node *p, boolean *found)
{
  node *q;
...
#if ! LVB	/* for LVB, avoid name clash with gdispose in cons.c */
...
#if ! LVB	/* for LVB, avoid name clash with reroot in cons.c */
...
void savetree(node *root, node *item, long *place, pointarray treenode,
...
{  /* adds item to tree and save it.  Then removes item. */
  ...
  savetree(*root, item, place, treenode, grbg, zeros);
  if (!multf)
...
void collabranch(node *collapfrom, node *collapto, node *tempfrom,
			node *tempto, long *zeros)
{ /* collapse branch between nodes collapfrom and collapto */
  long i, j, b, largest, descsteps;
  boolean done;

  for (i = 0; i < endsite; i++) {
    largest = getlargest(collapfrom->numnuc[i]);
    descsteps = 0;
    for (j = (long)A; j <= (long)O; j++) {
      b = 1 << j;
...
boolean passdown(node *desc, node *parent, node *start, node *below,
            ...
            node *tempprt, boolean multf, long *zeros)
...
boolean trycollapdesc(node *desc, node *parent, node *start,
  ...
  collabranch(desc, parent, tempdsc, tempprt, zeros);
  if (!allcommonbases(tempprt, parent, &allsame) || 
    ...
    do {
      if (p->back && !p->back->tip && 
        ...
        if (((p->back->collapse == tocollap) && (subtree->collapse == tocollap)
             && multf && (subtree != below)) || 
               trycollapdesc(p->back, subtree, start, below, item, added,
                            total, tempdsc, tempprt, multf, zeros))
          return true;
      }
      p = p->next;
    } while (p != subtree);
...
boolean collapsible(node *item, node *below, node *temp, node *temp1,
            ...
            boolean multf, long *zeros, pointarray treenode)
{
  ...
  if (multf && below->back && !below->back->tip) {
    ...
    multifillin(tempprt, tempdsc, -1);
    tempprt->numdesc += added->numdesc;
    collabranch(added, belowbk, tempdsc, tempprt, zeros);
...
void branchlength(node *subtr1, node *subtr2, node *root, double *brlen,
			long sitei, pointarray treenode)
{
...
void branchlentrav(node *p, node *root, long sitei, long chars,
  ...
  do {
    if (q->back) {
      branchlength(q, q->back, root, brlen, sitei, treenode);
...
#if ! LVB	/* for LVB, prevent name clash with coordinates and drawline
		 * in cons.c */
...
void copy_(tree *a, tree *b, long nonodes, long categs)
{
  ...
  for (i = spp; i < nonodes; i++) {
    if (a->nodep[i]) {
      ...
      } /* debug xx obviously the formatting will have to be fixed */
        }
        else
          q->back = NULL;
        p = p->next;
...
void standev2(long numtrees,long weightsum,long maxwhich,long a,long b,
		double maxlogl,double *l0gl,double **l0gf,steptr aliasweight)
{ /* compute and write standard deviation of user trees */
  /* used in dnaml & dnamlk */
  long j, num;
  double sum, sum2, TEMP, sd;

  fprintf(outfile, "Tree    Ln L      Diff Ln L     Its S.D.");
  fprintf(outfile, "   Significantly worse?\n\n");
  num = numtrees;
  for (which = 1; which <= num; which++) {
    fprintf(outfile, "%3ld%12.5f", which, l0gl[which - 1]);
    if (maxwhich == which)
      fprintf(outfile, "  <------ best\n");
    else {
      sum = 0.0;
...
/* debug   commented out because may be wrong to do this
  free(anode->numsteps);
  free(anode->oldnumsteps);
  free(anode->base);
  free(anode->oldbase);

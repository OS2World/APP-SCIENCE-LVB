...
extern long spp, nonodes, outgrno, nextree, which;
...
void getoptions()
{
  ...
  for (;;) {
   ...
   "\nAre these settings correct? (type Y or the letter for one to change)\n");
    ...
    if (ch == 'Y')
      break;
    if (strchr("CJOTUMI1234560",ch) != NULL){
      switch (ch) {
...
{ /* allocate remaining global arrays and variables dynamically */
  ...
  if (usertree) {
    ...
    for (i = 1; i <= maxuser; i++)
      fsteps[i - 1] = (long *)Malloc(chars*sizeof(long));
  }
			...
			boolean *bottom, sitearray nothing)
{
  ...
  prothyprint(b1, b2,bottom,r,&nonzero,&maybe,nothing);
  *bottom = false;
  if (!r->tip) {
